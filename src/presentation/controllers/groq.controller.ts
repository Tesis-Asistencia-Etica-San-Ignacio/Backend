...
import { getFileByNameBuffer} from "../../application";
...
export const processEvaluationController = async (req: Request, res: Response) => {
  try {
    ...
    const evaluationRepo = new EvaluacionRepository();
    const evaluation = await evaluationRepo.findById(evaluationId);
    ...
    /* Guardar en MongoDB
    await evaluationRepo.update(evaluationId, {
